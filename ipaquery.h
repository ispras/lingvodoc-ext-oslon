...
public:
	...
	public:
		...
		Segment(LPTSTR _txt)
		{
			...
			//обнул€ть как-то надо, тут нужен к-р особый дл€ Ђсовокупности признаковї?
	...
	bool Check_—“ј–џ…(Segmentizer* sgmtzr)
	{
		...
		{
			if (!CompareFeaturesOr(sdThis->feature, sgPrev.feature, FT_CLASS))//т.е. текуща€ годитс€ как контекст в принципе, но тольео по классу, т.с. — или √!!
				sgPrev.wasAlready++;
		}
		if (sgNext.flag != QF_NOTHING)
		{
			if (!CompareFeaturesOr(sdThis->feature, sgNext.feature, FT_CLASS))//т.е. текуща€ годитс€ как контекст в принципе
				sgNext.wasAlready++;
		}
		if (sgThis.flag != QF_NOTHING)
		{
			if (!CompareFeaturesOr(sdThis->feature, sgThis.feature, FT_CLASS))//т.е. текуща€ годитс€ как за€вленное Ђэтої
				sgThis.wasAlready++;
			else
				return false;
		}
	...
	bool Check(Segmentizer* sgmtzr)
	{
		...
		if (sgPrev.flag != QF_NOTHING)
		{
			if (!CompareFeaturesOr(sdThis->feature, sgPrev.feature, FT_CLASS))//т.е. текуща€ годитс€ как контекст в принципе, но только по классу, т.с. — или √!!
				sgPrev.wasAlready++;
		}
		if (sgNext.flag != QF_NOTHING)
		{
	...
	int GetFirstMatchingFragment(IPA* ipa, Sound** sound, LPTSTR wIn, LPTSTR wOut)
	{
		...
		*sound = sgmntzr.Current();
		...
		if (flags & QF_ITERATE)
		{
			...
			Sound* sPrev = sgmntzr.PeekPrevious(); //это пока так, ибо нет движени€ назад
			Sound* sNext;
			...
			while ((sNext = sgmntzr.GetNext()) && i <= nIterate)
			{
				...
				else if (sgThis.feature[FT_CLASS] == FT_VOWEL)//провер€ем потенциальные д-ги [надо это как-то обобщить]
				{
					...
					else if (sPrev)
					{
						...
						if (posFound) //слева направо
						{
							...
							//тут можно было бы замен€ть: wcscpy(wOut, _listExtraGlidesVowels[);
