class ComparisonWithGuess : public Comparison
{
	...
	int		nRowsNoCorresp;
public:
	ComparisonWithGuess(int _nRowsCorresp, int _nRowsNoCorresp, int nCols) : Comparison(_nRowsCorresp + _nRowsNoCorresp, nCols)
	{
		...
		while (parser.Next())
		{
			...
			if (iColIn >= begCols && iColIn < begCols + nCols)
			{
				...
				else
				{
					...
					if (wordIPA)
					{
						WordForm* wfNew = new (dic->pWordForms.New()) WordForm(wordIPA, wordOrig, wordTranslation, WF_HASLINK * (iRow > nRowsCorresp)); 
	...
	void Process()
	{
		...
		for (int iCol = 0; iCol < nDicts; iCol++)
		{
			...
			for (BTree::Walker w(&dic->trWordForms); word = (WordForm*)w.Next();)
			{
				...
				else if (iRowCorresp == nRowsAll - 1) goto Done;
			}
		}
	Done:
